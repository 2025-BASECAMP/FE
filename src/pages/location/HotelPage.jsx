...
import HotelCard from "../../components/location/PlaceCard";
...
const HotelPage = () => {
  ...
  const { list, loading, hasMore, error, lastElementRef, selectedLocation } =
    usePlaceList({ stadiumId: id, contentTypeId: 32 }); // 숙소
  ...
  return (
    <S.Container>
      ...
      {list.length === 0 && !loading ? (
        <EmptyState title="주변에 숙소를 찾지 못했습니다." description="다른 구장을 선택해보세요!" />
      ) : (
        <S.CardList>
          {list.map((item, idx) => (
            <HotelCard
      ...
      {loading && <Loading text="주변 숙소 탐색 중..." />}
      {!hasMore && list.length > 0 && (
        <EndMessage text={`총 ${list.length}개의 숙소를 모두 불러왔습니다`} />
      )}
    </S.Container>
  );
};
